...
if __name__ == "__main__":
    ...
    mobilenet_tflite = asnn('Electron')
    print(' |---+ ASNN Version: {} +---| '.format(mobilenet_tflite.get_nn_version()))

    print('Start init neural network ...')
    mobilenet_tflite.nn_init(library=library, model=model, level=level)
    print('Done.')
    ...
    outputs = mobilenet_tflite.nn_inference(cv_img, platform = 'TFLITE', reorder='0 1 2', output_format=output_format.OUT_FORMAT_FLOAT32)
    end = time.time()
